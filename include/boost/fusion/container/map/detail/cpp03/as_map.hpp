...
#define N BOOST_PP_ITERATION()
    ...
    struct as_map<N, is_assoc>
    {
        ...
        struct apply
        {
            ...
            typedef ::boost::fusion::map<BOOST_PP_ENUM_PARAMS(N, T)> type;
        };

        template <typename Iterator>
        BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED
        static typename ::boost::fusion::detail::as_map<N, is_assoc>
        ::template apply<Iterator>::type call(Iterator const& i0)
        {
            typedef typename ::boost::fusion::detail::as_map<N, is_assoc>
            ::template apply<Iterator> gen;
            typedef typename gen::type result;
            BOOST_PP_REPEAT(
                BOOST_PP_DEC(N), BOOST_FUSION_NEXT_CALL_ITERATOR, _
            )
            return result(
                BOOST_PP_ENUM(N, BOOST_FUSION_DREF_CALL_ITERATOR, _)
            );
        }
    };
...
BOOST_FUSION_BARRIER_BEGIN
    ...
    struct as_map<0, is_assoc>
    {
        ...
        static typename ::boost::fusion::detail::as_map<0, is_assoc>
        ::template apply<Iterator>::type call(Iterator)
        {
...
#pragma wave option(preserve: 2, line: 0, output: "preprocessed/as_map" \
...
#define BOOST_FUSION_NEXT_ITERATOR(z, n, data)                               \
    typedef typename ::boost::fusion::result_of::next<                       \
        BOOST_PP_CAT(I, n)                                                   \
    >::type BOOST_PP_CAT(I, BOOST_PP_INC(n));

#define BOOST_FUSION_NEXT_CALL_ITERATOR(z, n, data)                          \
    typename gen::BOOST_PP_CAT(I, BOOST_PP_INC(n))                           \
        BOOST_PP_CAT(i, BOOST_PP_INC(n)) = ::boost::fusion                   \
        ::next(BOOST_PP_CAT(i, n));

#define BOOST_FUSION_PAIR_FROM_ITERATOR(z, n, data)                          \
    typedef ::boost::fusion::detail                                          \
    ::pair_from<BOOST_PP_CAT(I, n), is_assoc> BOOST_PP_CAT(D, n);            \
