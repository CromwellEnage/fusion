...
============================================================================*/
#if !defined(BOOST_FUSION_ADAPTED_STRUCT_DETAIL_DEFINE_STRUCT_INLINE_HPP)
...
// the iterator class's 'deref' and 'value_of' metafunctions.  On these
// an alternate implementation for these metafunctions is used that does not 
// compilers require such specializations.  The alternate implementation takes
// longer to compile so its use is restricted to the offending compilers.
// For MSVC, the bug was reported at
// https://connect.microsoft.com/VisualStudio/feedback/details/757891/
// c-compiler-error-involving-partial-specializations-of-nested-templates
...
#if defined(BOOST_FUSION_NEED_NESTED_TEMPLATE_PARTIAL_SPEC_WKND)
...
// Note: all template parameter names need to be uglified,
//       otherwise they might shadow a template parameter of the struct
//       when used with BOOST_FUSION_DEFINE_TPL_STRUCT_INLINE
...
#define BOOST_FUSION_MAKE_ITERATOR_DEREF_SPEC(                               \
    ...
    {                                                                        \
        typedef typename boost_fusion_detail_Sq                              \
        ::t##N##_type TYPE_QUAL& type;                                       \
...
#define BOOST_FUSION_MAKE_AT_SPECS_AUX(R, DATA, N, ATTRIBUTE, IS_CONST)      \
    template <typename boost_fusion_detail_Sq>                               \
    struct at<boost_fusion_detail_Sq, ::boost::mpl::int_<N> >                \
    {                                                                        \
        typedef typename ::boost::mpl::if_<                                  \
            IS_CONST<boost_fusion_detail_Sq>                                 \
...
#define BOOST_FUSION_MAKE_AT_SPECS(R, DATA, N, ATTRIBUTE)                    \
    BOOST_FUSION_MAKE_AT_SPECS_AUX(R, DATA, N, ATTRIBUTE, ::boost::is_const)

#else

#define BOOST_FUSION_MAKE_AT_SPECS(R, DATA, N, ATTRIBUTE)                    \
    BOOST_FUSION_MAKE_AT_SPECS_AUX(R, DATA, N, ATTRIBUTE, ::std::is_const)
...
#define BOOST_FUSION_DEFINE_ITERATOR_DEREF_AUX(                              \
    NAME, ATTRIBUTES_SEQ, IS_CONST, ADD_CONST, REMOVE_CONST                  \
)                                                                            \
        template <typename boost_fusion_detail_Iterator>                     \
        struct deref                                                         \
        {                                                                    \
            ...
            static int const index = iterator_raw_type::index::value;        \
            typedef typename ::boost::fusion::result_of::at_c<               \
                ref_vec_t                                                    \
              , index                                                        \
            >::type result_raw_type;                                         \
            typedef typename ::boost::mpl::if_<                              \
                IS_CONST<typename iterator_raw_type::sequence_type>          \
              , typename ADD_CONST<result_raw_type>::type                    \
              , result_raw_type                                              \
            >::type type;                                                    \
            BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED                         \
...
#define BOOST_FUSION_MAKE_ITERATOR_WKND_REF_AUX(Z, N, DATA, IS_C, ADD_C)     \
        BOOST_PP_COMMA_IF(N)                                                 \
        typename ::boost::mpl::if_<                                          \
            IS_C<boost_fusion_detail_Seq>                                    \
          , typename ADD_C<                                                  \
...
#define BOOST_FUSION_DEFINE_ITERATOR_DEREF(NAME, ATTRIBUTES_SEQ)             \
    BOOST_FUSION_DEFINE_ITERATOR_DEREF_AUX(                                  \
        NAME                                                                 \
      , ATTRIBUTES_SEQ                                                       \
      , ::boost::is_const                                                    \
      , ::boost::add_const                                                   \
...
#define BOOST_FUSION_MAKE_ITERATOR_WKND_REF(Z, N, DATA)                      \
    BOOST_FUSION_MAKE_ITERATOR_WKND_REF_AUX(                                 \
        Z, N, DATA, ::boost::is_const, ::boost::add_const                    \
    )
...
#define BOOST_FUSION_MAKE_ITERATOR_WKND_REF(Z, N, DATA)                      \
    BOOST_FUSION_MAKE_ITERATOR_WKND_REF_AUX(                                 \
        Z, N, DATA, ::boost::is_const, ::std::add_const                      \
    )
...
#define BOOST_FUSION_MAKE_ITERATOR_WKND_REF(Z, N, DATA)                      \
    BOOST_FUSION_MAKE_ITERATOR_WKND_REF_AUX(                                 \
        Z, N, DATA, ::std::is_const, ::std::add_const                        \
    )

#endif  // BOOST_FUSION_USES_BOOST_VICE_CXX11_TYPE_TRAITS, BOOST_MSVC
...
#else   // !defined(BOOST_FUSION_NEED_NESTED_TEMPLATE_PARTIAL_SPEC_WKND)
...
//       ATTRIBUTES_SEQ may be empty and calling BOOST_PP_SEQ_SIZE
//       on an empty sequence produces warnings on MSVC.
...
#define BOOST_FUSION_DEFINE_STRUCT_INLINE_ITERATOR_IMPL_IMPL(                \
    ...
    {                                                                        \
        ...
        {                                                                    \
            ...
            static type                                                      \
            call(                                                            \
                boost_fusion_detail_It1 const& /* it1 */                     \
...
#define BOOST_FUSION_DEFINE_STRUCT_INLINE_MEMBERS_IMPL_IMPL(                 \
    ...
    NAME(boost_fusion_detail_Seq const& rhs)                                 \
    {                                                                        \
        ::boost::fusion::copy(rhs, *this);                                   \
    }                                                                        \
    template <typename boost_fusion_detail_Seq>                              \
    BOOST_CXX14_CONSTEXPR BOOST_FUSION_GPU_ENABLED                           \
    NAME& operator=(boost_fusion_detail_Seq const& rhs)                      \
    {                                                                        \
        ::boost::fusion::copy(rhs, *this);                                   \
        return *this;                                                        \
    }                                                                        \
