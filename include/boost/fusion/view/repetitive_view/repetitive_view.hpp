/*============================================================================
    Copyright (c) 2007 Tobias Schwinger

    Distributed under the Boost Software License, Version 1.0.
    (See accompanying file LICENSE_1_0.txt or copy at
    http://www.boost.org/LICENSE_1_0.txt)
============================================================================*/
#if !defined(BOOST_FUSION_REPETITIVE_VIEW_REPETITIVE_VIEW_HPP_INCLUDED)
#define BOOST_FUSION_REPETITIVE_VIEW_REPETITIVE_VIEW_HPP_INCLUDED

#include <boost/fusion/view/repetitive_view/repetitive_view_fwd.hpp>
#include <boost/fusion/view/repetitive_view/detail/begin_impl.hpp>
#include <boost/fusion/view/repetitive_view/detail/end_impl.hpp>
#include <boost/fusion/support/sequence_base.hpp>
#include <boost/fusion/support/is_view.hpp>
#include <boost/fusion/support/traversal_tags.hpp>
#include <boost/fusion/support/special_tags_fwd.hpp>
#include <boost/fusion/support/config.hpp>
#include <boost/mpl/bool.hpp>
#include <boost/mpl/if.hpp>

#if defined(BOOST_FUSION_USES_BOOST_VICE_CXX11_TYPE_TRAITS)
#include <boost/type_traits/remove_reference.hpp>
#else
#include <type_traits>
#endif

namespace boost { namespace fusion
{
    template <typename Sequence>
    struct repetitive_view :
        ::boost::fusion::sequence_base<
            ::boost::fusion::repetitive_view<Sequence>
        >
    {
        typedef ::boost::fusion::repetitive_view_tag fusion_tag;

        // This gets picked up by MPL.
        typedef ::boost::fusion::fusion_sequence_tag tag;

        typedef ::boost::mpl::true_ is_view;

        typedef ::boost::fusion::single_pass_traversal_tag category;

#if defined(BOOST_FUSION_USES_BOOST_VICE_CXX11_TYPE_TRAITS)
        typedef typename ::boost
#else
        typedef typename ::std
#endif
        ::remove_reference<Sequence>::type sequence_type;
        typedef typename ::boost::mpl::if_<
            ::boost::fusion::traits::is_view<Sequence>
          , Sequence
          , sequence_type&
        >::type stored_seq_type;

        BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED
        repetitive_view(Sequence& in_seq) : seq(in_seq)
        {
        }

        stored_seq_type seq;

     private:
        // silence MSVC warning C4512:
        // assignment operator could not be generated
        repetitive_view& operator=(repetitive_view const&);
    };
}}

#endif  // include guard

